...
class CustomContext(commands.Context):
    ...
    async def send(self, content: str = None, **kwargs):
        ...
        if self.guild and self.bot.config[self.guild.id]["owoify"]:
            ...
            if kwargs.get("embed"):
                kwargs["embed"] = owoified["embed"]

        if content and not len(content) > 1500 and random.randint(0, 100) == 50:
            message = (
                "You can join join the support server to catch up on "
                f"updates by doing: {self.prefix}support"
            )
            content += f"\n\n{message}"
...
class MyBot(commands.AutoShardedBot):
    ...
    async def blacklist_check(self, ctx: CustomContext):
        try:
            ctx.bot.blacklist[ctx.author.id]
        except KeyError:
            return True
